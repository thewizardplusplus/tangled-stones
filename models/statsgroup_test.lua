...
function TestStatsGroup.test_from_json_success()
  local stats_group, err = json.from_json(
    [[{
      "__name": "StatsGroup",
      "side_count": 42,
      "stats_by_side_count": {
  ...
  luaunit.assert_is_table(stats_group)
  luaunit.assert_is_true(checks.is_instance(stats_group, StatsGroup))

  luaunit.assert_is_number(stats_group._side_count)
  luaunit.assert_equals(stats_group._side_count, 42)

  luaunit.assert_is_table(stats_group._stats_by_side_count)
  luaunit.assert_equals(stats_group._stats_by_side_count, {
    ["23"] = Stats:new(0, 5290),
    ["42"] = Stats:new(0, 17640),
  })

  luaunit.assert_is_nil(err)
end

function TestStatsGroup.test_from_json_error()
  local stats_group, err = json.from_json(
    [[{
      "__name": "StatsGroup",
      "side_count": 42,
  ...
  luaunit.assert_is_nil(stats_group)

  luaunit.assert_is_string(err)
  luaunit.assert_str_matches(
    err,
    "^invalid data: " ..
      [[property "stats_by_side_count" validation failed: ]] ..
      [[failed to validate 23 %(matching "%^%[1%-9%]%%d%*%$"%): ]] ..
